...
from eval_2d import train_args
...
def get_parser():
    ...
    parser.add_argument("--output", default="eval_3d_output")
    parser.add_argument("--epoch", default=100, type=int)
    return parser


def construct_3d_test_dataset(args):
...
def eval(test_args, train_args, weight_path, log_dir, weight_fold):
    ...
    ) = construct_3d_test_dataset(test_args)
    (
        all_labels,
        labeled_paths,
        labeled_origins,
        unlabeled_paths,
        unlabeled_origins,
    ) = construct_3d_dataset(train_args)
    ...
    for fold in range(nb_folds):
        ...
        net = get_3d_net(train_args, n_classes)
